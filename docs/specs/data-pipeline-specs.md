...
## Normalization: Harford County Bills → Silver

- [x] **DATA-PIPE-013**: When normalizing a Harford County bill, the system shall set jurisdiction to COUNTY and body to "Harford County Council".
- [x] **DATA-PIPE-014**: When normalizing a Harford County bill, the system shall map status using HARFORD_STATUS_MAP (Introduced, Referred to Committee, In Committee, Passed, Adopted, Enacted, Failed, Withdrawn, Tabled, Pending), defaulting to UNKNOWN for unrecognized values via case-insensitive prefix matching.
- [x] **DATA-PIPE-015**: When normalizing a Harford County bill, the system shall infer item_type from the title: "resolution" in the title maps to RESOLUTION, "ordinance" maps to ORDINANCE, all others default to BILL.
